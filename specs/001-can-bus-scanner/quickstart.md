...
- A USB J2534-compatible adapter (e.g., Tactrix OpenPort 2.0, Drew Technologies MongoosePro) with its driver/provider installed — only USB adapters are supported; serial/Bluetooth/Wi-Fi adapters are out of scope
- Git (for cloning)
...
```powershell
.\build\src\cli\canmatik.exe monitor --provider "Tactrix OpenPort 2.0" --bitrate 500000
```
...
```powershell
.\build\src\cli\canmatik.exe monitor --provider "Tactrix OpenPort 2.0" --filter 0x7E8
```

### 4. Record to file

```powershell
.\build\src\cli\canmatik.exe record --provider "Tactrix OpenPort 2.0" --output captures/session1.asc
```
...
```powershell
.\build\src\cli\canmatik.exe record --provider "Tactrix OpenPort 2.0" --output captures/session1.jsonl --format jsonl
```
...
```powershell
.\build\src\cli\canmatik.exe monitor --provider "Tactrix OpenPort 2.0" --json
```

Pipe to `jq` for filtering:

```powershell
.\build\src\cli\canmatik.exe monitor --provider "Tactrix OpenPort 2.0" --json | jq 'select(.id == "7E8")'
```
